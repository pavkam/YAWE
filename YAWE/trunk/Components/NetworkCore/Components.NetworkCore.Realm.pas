...
type
  ...
  YNwBaseRealm = class(TBaseObject)
    private
  ...
  YNwRealmList = class(TBaseObject)
    private
      fList: TPtrArrayList;
...
begin
  Result := DataCore.Characters.CountEntries(FIELD_ACC_NAME, sAccName);
end;
...
begin
  inherited Create;
  sRType := SystemConfiguration.StringValue['Realm', 'Type'];

  fRealmType := ConvertRealmTypeToEnum(sRType);

  fMaxPopulation := SystemConfiguration.IntegerValue['Network', 'Players'];
  if fMaxPopulation = 0 then fMaxPopulation := $FFFF; { A big number }

  fTimeZone := SystemConfiguration.IntegerValue['Realm', 'TimeZone'];  //0; { U.S. TimeZone }

  fServer := SystemConfiguration.StringValue['Network', 'RealmAddress'];
  if fServer = '' then fServer := 'localhost'; { Default to this host }

  fPort := SystemConfiguration.IntegerValue['Network', 'RealmPort'];
  if fPort = 0 then fPort := 3725; { Default Realm Port }

  fName := SystemConfiguration.StringValue['Realm', 'Name'];
