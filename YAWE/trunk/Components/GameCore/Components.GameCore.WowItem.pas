...
type
  ...
  YGaItem = class(YGaWowObject, IObject, IItem)
    ...
    public
      ...
      property Template: YDbItemTemplate read fTemplate;
...
procedure YGaItem.ExtractObjectData(cEntry: YDbSerializable);
var
  iInt: Int32;
  cItm: YDbItemEntry;
begin
  ...
  begin
    ...
    begin
      SetUInt32(ITEM_FIELD_SPELL_CHARGES + iInt, fTemplate.Spells[iInt].Charges);
    end;
...
procedure YGaItem.InjectObjectData(cEntry: YDbSerializable);
var
  iStackCount: Int32;
  cItm: YDbItemEntry;
begin
  ...
  cItm := YDbItemEntry(cEntry);
  cItm.Entry := GetUInt32(OBJECT_FIELD_ENTRY);
  cItm.ContainedIn := GetUInt32(ITEM_FIELD_CONTAINED);
...
begin
  if fTemplate <> nil then DataCore.ItemTemplates.ReleaseEntry(fTemplate);
  DataCore.ItemTemplates.LoadEntry(iTempId, fTemplate);
end;
