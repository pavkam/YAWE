...
{$ENDREGION}
  ...
  YGaEquipMgr = class(YGaPlayerComponent)
    ...
    public
      ...
      procedure SetInventorySlotHead(iSlot : UInt32; iBag : UInt32 = BAG_NULL);
  ...
  YGaChatMgr = class(YGaPlayerComponent)
    private
      ...
      { Guild test commands }
      function CommandCreateGuild(Sender: YGaCommandHandlerTable; cParams: TStringDataList): YCommandCallResult;
      function CommandGuildReport(Sender: YGaCommandHandlerTable; cParams: TStringDataList): YCommandCallResult;
  ...
  YGaGuildMgr = class(YGaPlayerComponent)
    ...
    public
      ...
      property Inv: UInt32 read fInvite write fInvite;
...
begin
  ...
  SetInventorySlotHead(iSlot1, iBag1);
  SetInventorySlotHead(iSlot2, iBag2);
...
begin
  ...
  begin
    ...
    SetInventorySlotHead(iSlot, iBag);
...
begin
  ...
  /// Settinh Inventory Slot Head UPDATE VALUE
  ///  We presume that this method is called actually only for item in inventory
  ///  (equipment or backpack) and not for items in bags or bank slot or w/e :)

  SetInventorySlotHead(iSlot, BAG_NULL);
...
{*------------------------------------------------------------------------------
  This method prepares the Inventory Slot Head updates for the item on given slot

  @param iSlot
-------------------------------------------------------------------------------}
procedure YGaEquipMgr.SetInventorySlotHead(iSlot: UInt32; iBag : UInt32);
const
  { Constants used for deleted item settings, 0 values }
  __ITEM_GUID     = 0;
...
begin
  ...
  if (iBag = BAG_NULL) then
    ...
    cTemp := fItems.Containers[iBag].Items[iSlot];
  ...
  begin
    ...
    begin
      /// Adding Item's GUID in proper slot ID in update fields
      Owner.SetUInt64(CONTAINER_FIELD_SLOT_1 + iSlot * 2, cTemp.GUID^.Full);
    end else
    begin
      Owner.SetUInt64(CONTAINER_FIELD_SLOT_1 + iSlot * 2, __ITEM_GUID);
    end;
  end
...
begin
  ...
  case iType of
    ...
    CHAT_MSG_GUILD:
    begin
      if Owner.Guild.Entry <> nil then
      begin
        sMessage := cPkt.ReadString;
        iLen := Length(sMessage);
        cOutPkt := BuildChatPacket(Owner, CHAT_MSG_GUILD, glUniversal,
        Pointer(sMessage), iLen);
        try
          YGaGuild(Owner.Guild.Entry).SayToGuild(Owner, cOutPkt);
        finally
          cOutPkt.Free;
        end;
      end;
    ...
    begin
      if Owner.Guild.Entry <> nil then
        ...
        sMessage := cPkt.ReadString;
        iLen := Length(sMessage);
        cOutPkt := BuildChatPacket(Owner, CHAT_MSG_OFFICER, glUniversal,
        Pointer(sMessage), iLen);
        try
          YGaGuild(Owner.Guild.Entry).SayToOfficer(Owner, cOutPkt);
        finally
          cOutPkt.Free;
        end;
      end;
...
function YGaChatMgr.CommandCreateGuild(Sender: YGaCommandHandlerTable;
  cParams: TStringDataList): YCommandCallResult;
var
  Name: string;
begin
  ...
  GameCore.CreateNewGuild(Owner, Name);

  Result := ccrSupressMsg;
end;

function YGaChatMgr.CommandGuildReport(Sender: YGaCommandHandlerTable;
  cParams: TStringDataList): YCommandCallResult;
var
  X,Y: UInt32;
  cOutPkt: YNwServerPacket;
...
begin
  ...
  //writeln(GameCore.LookForGuild(cParams.GetAsString(0)));
...
const
  ...
  HELP_CTX_GUILD_CREATE = 'Creates new guild';
...
begin
  ...
  GameCore.CommandHandler.AddCommandHandler(['ADD', 'GUILD'], @YGaChatMgr.CommandCreateGuild, [1, 2], HELP_CTX_GUILD_CREATE, SYN_GUILD_CREATE, []);
  GameCore.CommandHandler.AddCommandHandler(['ADD', 'R'], @YGaChatMgr.CommandGuildReport, [1, 2], HELP_CTX_GUILD_CREATE, SYN_GUILD_CREATE, []);
