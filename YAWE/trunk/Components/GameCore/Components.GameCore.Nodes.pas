...
type
  ...
  YGaNodeManager = class(TBaseInterfacedObject, INodeManager)
    private
      fNodes: TIntHashMap;
...
type
  ...
  YGaNode = class(TBaseInterfacedObject, INode)
    private
      fId: UInt32;
      fPosition: TVector;
      fMapId: UInt32;
    ...
    protected
      ...
      procedure ExtractObjectData(Entry: YDbSerializable);
      { Override this in children to perform resource saving }
      procedure InjectObjectData(Entry: YDbSerializable);
    public
  ...
  YGaNodeContext = class(TBaseReferencedObject, INodeContext, IDofStreamable)
    private
      fOwner: INode;
...
begin
  ...
  LoadFromDatabase;
  fNodesUpdateHandle := SystemTimer.RegisterEvent(OnNodesUpdate, 1000,
...
procedure YGaNode.InjectObjectData(Entry: YDbSerializable);
var
  cNode: YDbNodeEntry;
  iInt: Int32;
begin
  cNode := YDbNodeEntry(Entry);
  cNode.X := fPosition.X;
  cNode.Y := fPosition.Y;
  cNode.Z := fPosition.Z;
  cNode.MapId := fMapId;
  ...
  begin
    (fContexts[iInt] as INodeContext).InjectObjectData(cNode as INodeEntry);
  end;
end;

procedure YGaNode.ExtractObjectData(Entry: YDbSerializable);
var
  cNode: YDbNodeEntry;
  iInt: Int32;
begin
  ...
  for iInt := 0 to fContexts.Size -1 do
  begin
    (fContexts[iInt] as INodeContext).ExtractObjectData(cNode as INodeEntry);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      Entry.&Object := YGaMobile(GameCore.CreateObject(otUnit, False));
      YGaCreature(Entry.&Object).CreateFromTemplate(Entry.EntryId);
    end else
    begin
      Entry.&Object := YGaMobile(GameCore.CreateObject(otGameobject, False));
      YGaGameObject(Entry.&Object).CreateFromTemplate(Entry.EntryId);
    end;
    ...
    Entry.&Object.Position.Vector := tRandVec;
    Entry.&Object.Position.Angle := Random * (2 * PI);
    Entry.&Object.Position.MapId := fOwner.MapId;
