...
type
  ...
  YGameCore = class(YBaseCore)
    private
      ...
      function GuildById(Index: UInt32): YGaGuild;
    protected
    ...
    public
      ...
      (*
        Guild Control Utilities
      *)

      { Creates new guild from ashes }
      procedure CreateNewGuild(cPlayer: YGaPlayer; Const Name: String);
      procedure DeleteGuild(ID: UInt32);
      property Guilds[Index: UInt32]: YGaGuild read GuildById;
...
begin
  ...
  fGuilds := TIntPtrHashMap.Create(10);
...
function YGameCore.GuildById;
begin
  Result := fGuilds.GetValue(Index);
end;

procedure YGameCore.CreateNewGuild;
var
  nGuild: YGaGuild;
  iInt: UInt32;
begin
  for iInt := 1 to 100 do
    if not fGuilds.ContainsKey(iInt) then
    begin
      nGuild := YGaGuild.Create(cPlayer, iInt, Name);
      fGuilds.PutValue(iInt, nGuild);
      break;
    end;

  cPlayer.Guild.Rank := 0;
  cPlayer.Guild.Inv := 0;
  cPlayer.Guild.Entry := nGuild;
  cPlayer.Guild.ID := iInt;

end;

procedure YGameCore.DeleteGuild;
begin
  fGuilds.Remove(ID);
end;
