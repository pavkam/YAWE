...
type
  ...
  YGaMobile = class(YGaWowObject)
    ...
    protected
      ...
      procedure SetBackFlySpeed(const Value: Float);
      procedure SetFlySpeed(const Value: Float);
    ...
    public
      ...
      property FlySpeed: Float read fFlySpeed write SetFlySpeed;
      property BackFlySpeed: Float read fBackFlySpeed write SetBackFlySpeed;
      ...
      procedure SendRootValue(iValue : UInt32);
      procedure SendUnroot();
  end;
...
var
  X, Y, Z: Int32;
begin
  X := Obj1.ApproxX - Obj2.ApproxX;
  Y := Obj1.ApproxY - Obj2.ApproxY;
  Z := Obj1.ApproxZ - Obj2.ApproxZ;
  Result := X * X + Y * Y + Z * Z;
end;
...
begin
  ...
  cArea := GameCore.GetArea(MapId, X, Y);
  AreaX := cArea.X;
  AreaY := cArea.Y;
  for iInt := 0 to cArea.ObjectCount -1 do
  begin
    if IsVectorDistanceWithin(Vector, cArea.Objects[iInt].Vector, VIEW_DISTANCE) and CanSee(cArea.Objects[iInt]) then
    begin
...
procedure YGaMobile.ExtractObjectData(const Entry: ISerializable);
begin
  inherited;
...
begin
  ...
  cArea := GameCore.Areas[MapId, AreaX, AreaY];
...
begin
  ...
  begin
    while (VectorDistanceSq(aArr[Result].Vector, Vector) <= RangeSq) and
          (Result < High(aArr)) do
    begin
      Inc(Result);
    end;
...
procedure YGaMobile.SetMapId(const Value: UInt32);
begin
  fMap := Value;
end;

procedure YGaMobile.SendRootValue(iValue: UInt32);
var
 cOutPacket: YNwServerPacket;
begin
  cOutPacket := YNwServerPacket.Initialize(SMSG_FORCE_MOVE_ROOT);
  cOutPacket.AddPackedGUID(GUID);
  cOutPacket.AddUInt32(iValue);
  //SendPacket(cOutPacket);
end;

procedure YGaMobile.SendUnroot;
var
 cOutPacket: YNwServerPacket;
begin
  //cOutPacket := YNwServerPacket.Initialize(SMSG_FORCE_MOVE_UNROOT);
  //cOutPacket.AddPackedGUID((fOwner as YGaPlayer).GUID);
  //(fOwner as YGaPlayer).Session.SendPacket(cOutPacket);
end;

procedure YGaMobile.SetAngle(const Value: Float);
begin
  fAngle := Value;
end;

procedure YGaMobile.SetAreaGridX(val: UInt32);
begin
  fAreaGridX := Val;
end;

procedure YGaMobile.SetBackFlySpeed(const Value: Float);
begin
  fBackFlySpeed := Value;
end;

procedure YGaMobile.SetFlySpeed(const Value: Float);
begin
  fFlySpeed := Value;
end;

procedure YGaMobile.SetX(const Value: Float);
begin
  fVec.X := Value;
  fXApprox := Floor32(Value);
end;

procedure YGaMobile.SetY(const Value: Float);
begin
  fVec.Y := Value;
  fYApprox := Floor32(Value);
end;

procedure YGaMobile.SetZ(const Value: Float);
begin
  fVec.Z := Value;
  fZApprox := Floor32(Value);
end;

procedure YGaMobile.SetSpeedBackSwim(const Value: Float);
begin
  fBackSwimSpeed := Value;
end;

procedure YGaMobile.SetSpeedBackWalk(const Value: Float);
begin
  fBackWalkSpeed := Value;
end;

procedure YGaMobile.SetSpeedRun(const Value: Float);
begin
  fRunSpeed := Value;
end;

procedure YGaMobile.SetSpeedSwim(const Value: Float);
begin
  fSwimSpeed := Value;
end;

procedure YGaMobile.SetSpeedTurnRate(const Value: Float);
begin
  fTurnRate := Value;
end;

procedure YGaMobile.SetSpeedWalk(const Value: Float);
begin
  fWalkSpeed := Value;
end;

procedure YGaMobile.SetVector(const Vec: TVector);
begin
  fVec := Vec;
...
procedure YGaMobile.SetZoneId(const Value: UInt32);
begin
  fZone := Value;
end;

function YGaMobile.GetAngle: Single;
begin
  Result := fAngle;
end;

function YGaMobile.GetX: Single;
begin
  Result := fVec.X;
end;

function YGaMobile.GetY: Single;
begin
  Result := fVec.Y;
end;

//procedure YGaMobile.GetCurrentPosition(var tPos: YWorldPosition);
//begin
  //Move(fX, tPos, SizeOf(YWorldPosition));
//end;

function YGaMobile.GetMapId: Longword;
begin
  Result := fMap;
end;

function YGaMobile.GetZ: Single;
begin
  Result := fVec.Z;
end;

function YGaMobile.GetZoneId: Longword;
begin
  Result := fZone;
end;
...
procedure YGaMobile.InjectObjectData(const Entry: ISerializable);
begin
  inherited;

end;

procedure YGaMobile.DoTeleport(iMap: UInt32; const fX, fY, fZ: Float);
var
  cOutPkt: YNwServerPacket;
begin
  if MapId = iMap then
  begin
    ...
    try
      ...
      SendPacketInRange(cOutPkt, VIEW_DISTANCE, True, True);
    finally
      cOutPkt.Free;
    end;
  end else
