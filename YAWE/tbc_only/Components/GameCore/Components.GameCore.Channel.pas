...
type
  ...
  YGaChannel = class(TInterfacedObject)
    protected
      fName: string;
      fPassword: string;
      fOwner: YGaPlayer;
...
begin
  ...
  begin
    ...
    begin
      ...
      if iRes = 1 then cTarget.SendSystemMessage('You were unbanned from channel ' + fName + '.');
    end;
  end else
  begin
    cOutPkt := PrepareChannelPacket(CHN_NOTIFY_NOT_MOD);
...
begin
  ...
  begin
    ...
    end else cPlayer.SendSystemMessage('A static channel cannot have an owner.');
  end else
  begin
