...
begin
  ...
  {$IFDEF WOW_TBC}
  cOutPkt := YNwServerPacket.Initialize(CMSG_DUNGEON_DIFFICULTY);
  cOutPkt.Jump(4);
  cOutPkt.AddUInt8(1);
  cOutPkt.Jump(7);
  SendPacket(cOutPkt);

  cOutPkt := YNwServerPacket.Initialize(SMSG_LOGIN_VERIFY_WORLD);
  cOutPkt.Jump(20);
  SendPacket(cOutPkt);
  {$ENDIF}
  ...
  {$IFDEF WOW_TBC}
  cOutPkt := YNwServerPacket.Initialize(SMSG_BROADCAST_MSG);
  cOutPkt.AddUInt32(6);
  cOutPkt.AddString(PatchNotesBroadcast);
  SendPacket(cOutPkt);
  {$ENDIF}
  ...
  {$IFDEF EXPERIMENTAL}
  cOutPkt := YNwServerPacket.Initialize(SMSG_MOVE_UNLOCK_MOVEMENT);
  cOutPkt.JumpUInt32;
  SendPacket(cOutPkt);


  fLoggedInPlayer.Position.SendRootValue(PLAYER_MOVEMENT_UNLOCKED);
  fLoggedInPlayer.Position.SendUnroot();
  {$ENDIF}
