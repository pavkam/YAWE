...
type
  ...
  YNwWorldSocket = class(YIoClientSocket, ISocketInterface)
    private
      ...
      procedure HandleAUnk206Opcode(cPkt: YNwClientPacket);
      { Ping event  }
...
procedure YNwWorldSocket.HandleAUnk206Opcode(cPkt: YNwClientPacket);
var
 aUnkValue : UInt32;
 cOutPkt: YNwServerPacket;
begin
 aUnkValue := cPkt.ReadUInt32;

 cOutPkt := YNwServerPacket.Initialize(SMSG_NEW_WOW_TO_TBC_OPC_REALM);
 cOutPkt.AddUInt32(aUnkValue);
 cOutPkt.JumpUInt32();
 cOutPkt.AddString('01/01/01');
