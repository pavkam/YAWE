...
type
  ...
  TRawClientSocket = class(TBaseInterfacedObject)
    protected
      fSocket: TSocket;
      fAddress: TSockAddr;
  ...
  TSocketServer = class(TBaseInterfacedObject)
    private
...
begin
  ...
  begin
    fThreads[iInt] := SystemThreadManager.CreateThread(ProcessNetworkIo, True,
      'Network_Io_Thread');
  end;

  for iInt := NumOfIoThreads to fThreadCount -1 do
  begin
    fThreads[iInt] := SystemThreadManager.CreateThread(ProcessAcceptRequests, True,
      'Accept_Io_Thread');
  end;
  ...
  fTimeoutCheckThread := SystemThreadManager.CreateThread(CheckConnectionsForTimeout,
...
begin
  ...
  begin
    fThreads[iInt].WaitForEnd(-1);
  end;

  fTimeoutThreadEvent.Signal;
  fTimeoutCheckThread.WaitForEnd(-1);
...
begin
  ...
  fTimer := SystemTimer.RegisterEvent(ClearBannedAndIpLists, BANNED_CLEAR_PERIOD,
