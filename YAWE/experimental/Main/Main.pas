...
begin
  ...
  SystemDebugger := TExceptionWatcher.Create;
  SystemTypeRegistry := TTypeRegistry.Create;
  SystemSerializationRegistry := TSerializationRegistry.Create;
  SystemThreadManager := TThreadManager.Create;
  SystemLogger := TLogManager.Create;
  SystemTimer := TTickGenerator.Create(TICK_LENGTH);
  SystemConfiguration := TConfiguration.Create(ConfigurationFile);
  SystemOptimizer := TOptimizationManager.Create;

  SystemTypeRegistry.RegisterType(TypeInfo(TExceptionFlag));
  SystemTypeRegistry.RegisterType(TypeInfo(TExceptionFlags));
end;

procedure FinalizeFramework;
begin
  FreeAndNil(SystemOptimizer);
  FreeAndNil(SystemConfiguration);
  FreeAndNil(SystemTimer);
  FreeAndNil(SystemLogger);
  FreeAndNil(SystemThreadManager);
  FreeAndNil(SystemSerializationRegistry);
  FreeAndNil(SystemTypeRegistry);
  FreeAndNil(SystemDebugger);
end;
...
begin
  aThreads := SystemThreadManager.EnumThreadsInfo;
...
begin
  ...
  begin
    if SystemThreadManager.GetThreadInfo(Args[0], tThread) then
    begin
...
function ProcessWindowMessages(pData: PThreadData): Longword;
var
  tWinMsg: TMsg;
begin
  ...
  bInTray := False;
  FillChar(tWinMsg, SizeOf(tWinMsg), 0);

  while not bShuttingDown do
  begin
    while PeekMessage(tWinMsg, hInvisibleWindow, 0, 0, PM_REMOVE) do { Process pending messages }
    begin
      TranslateMessage(@tWinMsg);
      DispatchMessage(@tWinMsg); { Dispatch them to the WndProc }
    end;
    WaitMessage;
  end;
...
begin
  ...
  // Optimizing routines
  //------------------------------------------------------------------------------
  SystemOptimizer.AddFunctionVariant(@XorBuffers, @XorBuffersMMX, cisMMX);
  SystemOptimizer.AddFunctionVariant(@AndBuffers, @AndBuffersMMX, cisMMX);
  SystemOptimizer.AddFunctionVariant(@OrBuffers, @OrBuffersMMX, cisMMX);
  SystemOptimizer.AddFunctionVariant(@AndNotBuffers, @AndNotBuffersMMX, cisMMX);
  SystemOptimizer.AddFunctionVariant(@NotBuffer, @NotBufferMMX, cisMMX);

  SystemOptimizer.AddFunctionVariant(@System.Move, @MoveMMX, cisMMX);
  SystemOptimizer.AddFunctionVariant(@System.Move, @MoveSSE, cisSSE);
  SystemOptimizer.AddFunctionVariant(@System.Move, @MoveSSE2, cisSSE2);
  SystemOptimizer.AddFunctionVariant(@System.Move, @MoveSSE3, cisSSE3);

  { And patch it }
  SystemOptimizer.PatchFunctions;
  ...
  try
    ...
    on cEx: EFileLoadError do
    begin
    end;
  end;

  SetGlobalCompressionLevel(SystemConfiguration.IntegerValue['Data', 'PacketCompressionLevel']);

  SystemTimer.StartActivity;

  SystemTimer.Threads[TICK_EXGROUP_DEFAULT].WaitForStart(-1);

  tShutDown.Init(False, False);

  New(pData);
  StartThreadAtAddress(@ProcessWindowMessages, nil, True, pData^);
  ThreadPushCleanupRoutine(pData^.Handle, @ThreadCleanupProc, pData);
  ThreadModifyState(pData^.Handle, False);
  ...
  try
    ...
    IoCore.Console.WriteStartupTime(StopExecutionTimer(liCounter));
    ...
    SystemTimer.StopActivity; { Wait for the tick generator thread to end }
    ...
    IoCore.Console.WriteShutdownTime(StopExecutionTimer(liCounter));
