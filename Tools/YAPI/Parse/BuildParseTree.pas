...
begin
  ...
  begin
    ...
    if CanFollowTAGComment(peTokenTypes) then
     ...
     begin
       if IsCurlyComment(fcTokenList.First) then RecogniseCurlyBlock
         ...
         else if IsSlashComment(fcTokenList.First) then RecogniseSlashLine(false)
   ...
   begin
     ...
     begin
       if IsCurlyComment(fcTokenList.First) then RecogniseCurlyBlock
         ...
         else if IsSlashComment(fcTokenList.First) then RecogniseSlashLine(true)
          else Recognise(ttComment);
     end;     
   end;
  Inc(fiTokenCount);
...
begin
  ...
  begin
    ...
    Result := (lc.WordType = wtReservedWord) or (lc.WordType in IdentifierTypes);
    Exit;
  end;
...
begin
  ...
  { or experimental }
  if fcTokenList.FirstSolidTokenType = ttExperimental then
    Recognise(ttExperimental);
...
begin
  ...
  Recognise(ttBegin);
  // TridenT
  inc(fbInsideCpdStatmt);//:=true;
  RecogniseStatementList([ttEnd]);
  Recognise(ttEnd);
  // TridenT
  dec(fbInsideCpdStatmt);//:=false;
  PopNode;
end;
    ...
    case lc.TokenType of
      ...
      begin
        ...
        case fcTokenList.SolidTokenType(2) of
          ...
          ttProperty:
          begin
            // TridenT - Removed line under, class recognized inside RecogniseProperty()
            // Recognise(ttClass);
            RecogniseProperty;
          end;
          ttConstructor:
...
begin
  ...
  // TridenT - class property
  if fcTokenList.FirstSolidTokenType = ttClass then Recognise(ttClass);
...
const
  ASM_OPERATORS = [ttPlus, ttMinus, ttAnd, ttOr, ttTimes, ttFloatDiv, ttPtr, ttColon, ttShl, ttShr];
  ...
  case fcTokenList.FirstSolidTokenType of
    ...
    begin
     ...
     { while true do
      ...
      if (lcNext.TokenType = ttIdentifier) and (lcNext.SourceCode[Length(lcNext.SourceCode)] = 'h') then
      begin
        Recognise(ttIdentifier);
      end;
